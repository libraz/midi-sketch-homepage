...
// Ref for stepper container (for auto-scroll on mobile)
const stepperRef = ref<HTMLElement | null>(null)

// Scroll step to appropriate position based on step index
function scrollStepToPosition(step: Element, stepIndex: number, totalSteps: number) {
  ...
  // Calculate target position based on progress (0 = left, 0.5 = center, 1 = right)
  const progress = totalSteps > 1 ? stepIndex / (totalSteps - 1) : 0.5
  // Map progress to container position (20% to 80% of container width)
  const targetPosition = containerRect.left + containerRect.width * (0.2 + progress * 0.6)

  const stepCenter = stepRect.left + stepRect.width / 2
  const scrollOffset = stepCenter - targetPosition
...
watch(() => store.currentStep.value, (newStep) => {
  // Stop playback when changing steps
  stopPlayer()
  isAnimating.value = true
  setTimeout(() => isAnimating.value = false, 400)

  // Auto-scroll stepper to current step on mobile (after DOM update)
  nextTick(() => {
    if (stepperRef.value) {
      const activeStep = stepperRef.value.querySelector('.noir-stepper__step--active')
      if (activeStep) {
        const stepIndex = newStep - 1 // Convert 1-based to 0-based index
        scrollStepToPosition(activeStep, stepIndex, steps.value.length)
      }
    }
  })
})
...
<template>
  <div class="noir-wizard">
    ...
    <!-- Stepper -->
    <nav ref="stepperRef" class="noir-stepper" role="tablist">
...
@media (max-width: 640px) {
  ...
  /* Add padding to first/last step for edge spacing */
  .noir-stepper__step:first-child {
    padding-left: 1rem;
  }

  .noir-stepper__step:last-child {
    padding-right: 1rem;
  }
