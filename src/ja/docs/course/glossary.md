...
| 度数 | スケール内での音の位置（第1、第2、…） | — |
| 音程 | 2つの音高の距離。スケールのステップ数で名付ける（3度・5度）か、半音数で数える | — |
| 平行調（レラティブマイナー） | メジャーキーと同じ7音を共有するマイナーキー | `key`、`mood` |
| 導音 | 主音の半音下の音。`V`→`I` のカデンツの引力の源 | — |
...
| コードの性質（クオリティ） | メジャー、マイナー、ディミニッシュなど、コードの色 | — |
| コードシンボル | コードの文字名、例 `Am`、`G7` | — |
| ローマ数字表記 | 度数でラベル付けしたコード、例 `I`、`V`、`vi` | — |
| ダイアトニック | 現在のキーの7音（または7つのコード）に属すること | — |
...
| 借用和音（`♭VII`） | 同主調から引き込んだコード。例 `I-♭VII-IV-I` | `chordProgressionId`（Rockプリセット） |
| プラガル（変格終止） | 導音を伴わない `IV`→`I` 型の解決 | `chordProgressionId`（Rockプリセット） |
| ミクソリディアン | 第7音を半音下げたメジャースケール。♭VII の出どころ | — |
...
| ブリッジ | 曲の後半に一度だけ現れる対照的なセクション | `formId` |
...
| 転調（キーチェンジ） | 曲全体を新しいキーへ移すこと。多くは最後のサビを半音1〜2個分上げる | `modulationTiming`、`modulationSemitones` |
