...
::: tip 音楽理論が初めての方へ
このページはキー・進行・フック・楽曲フォームといった音楽用語で機能を説明します。馴染みのないものがあれば、[コース](/ja/docs/course/primer)が各機能の背景にある基礎を再生できる譜例で解説します。
:::
...
- 回避音の検出。判定はコードルートからの音程で行い、和声機能によって切り替わります。メジャートニック上の完全4度、マイナーコード上の短6度、および短2度と長7度が対象です。トライトーンはトニックやサブドミナント上では回避音ですが、ドミナント上では解決の核となるため必須です。
:::
...
::: info 強拍と弱拍
4/4拍子では、1拍目と3拍目が**強拍**、2拍目と4拍目が**中拍**、裏拍の8分音符位置が**弱拍**、その他の16分音符位置が**最弱拍**です。メロディは通常、和声的な明確さのために強拍にコードトーンを置きます。
:::

::: details NCTタイプ
| タイプ | 配置 | 説明 |
|--------|------|------|
| 経過音 | 強拍以外 | コードトーン間を順次進行でつなぐ |
| 刺繍音 | 強拍以外 | コードトーンから離れて戻る |
| 倚音 | 強拍 | 順次解決するアクセント付き不協和 |
| 先取音 | 拍前 | 次のコードトーンの早期到着 |
| 掛留音 | コード変化点 | 前のコードの音を保留し、順次下行で解決 |
| テンション | 文脈依存 | 9th, 11th, 13th エクステンション |
:::

::: details ムード依存の設定
- **Bright/Upbeat、Idol、Anthem**: 71%コードトーン、ペンタトニック重視
- **Energetic/Dance、Light Rock、Future Bass**: 77%コードトーン、リズム重視
- **Chill、Synthwave**: 66%コードトーン
- **Dark/Dramatic、Nostalgic**: 57%コードトーン、クロマチックアプローチ音有効
- **Ballad、Sentimental、Emotional Pop**: 52%コードトーン、表現力豊かな倚音、テンション有効
- **CityPop**: 48%コードトーン、ジャズテンションとクロマチックアプローチ有効
:::

### ハーモニーコンテキスト＆衝突回避

::: details マルチトラック協調
- **トラック衝突検出**: Vocal、Chord、Bass、Motif、Aux、Guitar、Arpeggioの全ピッチトラックの音を登録
- **低音域ガード**: 両方の音がC4未満の場合は短い経過音の許容を無効にして濁りを防ぎます。一般的な3半音の閾値ではありません
...
基本の Up、Down、UpDown、Random に加えて、以下のパターンがあります。
- **Pinwheel**: 方向が交互に変わるパターン
- **PedalRoot**: 各音の間にルートに戻る
- **Alberti**: クラシック的な分散和音パターン（低-高-中-高）
- **BrokenChord**: ルート-3度-5度-オクターブと上行し、5度・3度を経て下行
:::
...
- **SyllabicSubRate**: `0` はスタイル既定値を使い、`1`〜`100` は音節分割比率をパーセントで上書き
:::
...
`--style` にはスタイルプリセットID（0〜16）を指定します。
...
メロディ生成では、最初の結果をそのまま出力しません。セクションごとに **20〜100個の候補** を生成し、評価を通じて 1 つを選択します：

1. **スコアリング**：全候補を、スタイル品質・ペナルティによる歌いやすさ・音程配分の適合度で採点
2. **カリング**：スコア下位半分を破棄。高音域の負担、単調さ、散らばった音は順位を下げます
3. **選択**：残った半分からスコアに比例した重み付きランダムで決定。高スコアが選ばれやすい一方、繰り返しごとに同じメロディへ収束しません
...
|---------|:-----:|:---:|:-----:|:--------:|------|
| **MelodyLead (0)** | Yes | Yes | パラダイム/RiffPolicy/Blueprint依存 | Optional | 歌ありの曲 |
| **BackgroundMotif (1)** | No | Yes | Enabled | Optional | BGM、Lo-Fi |
| **SynthDriven (2)** | No | No | Enabled | Optional（手動有効化） | エレクトロニック、EDM |

::: warning BGM専用モード
BackgroundMotifはVocalを無効にし、Auxを有効にしたままMotif生成を有効にします。SynthDrivenはVocalとAuxの両方を無効にしてMotif生成を有効にします。セクションマスクとレイヤースケジュールによってMotifノートが残る場所が決まり、Arpeggioは`arpeggioEnabled=true`で手動で有効にする必要があります。
:::
...
::: info ライセンス
AGPL-3.0 / 商用のデュアルライセンス。生成したMIDIはそのまま商用利用できます。エンジン本体は AGPL-3.0 のもとで自由に利用・改変・再配布でき、クローズドソース製品やプロプライエタリな SaaS への組み込みには商用ライセンスが必要です。
:::
