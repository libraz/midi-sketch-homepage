...
**ソース:** `src/track/vocal.cpp`（約960行）、`src/track/melody_designer.cpp`（約1280行）
...
ボーカルトラックはメロディックエンベリッシュメントシステムを使用し、コードトーンメロディに音楽的な「遊び」を追加します：

| NCTタイプ | 説明 | 配置 |
|----------|------|------|
| **ChordTone** | 和声音（基準） | 強拍 |
| **PassingTone** | コードトーン間の順次進行 | 弱拍 |
| **NeighborTone** | 同じコードトーンへ戻る装飾 | 弱拍 |
| **Appoggiatura** | 順次解決する強調された不協和音 | 強拍 |
| **Anticipation** | 次のコードトーンの先取り | コードチェンジ前 |
| **Tension** | コード拡張からの9度、11度、13度 | スタイルに依存 |
