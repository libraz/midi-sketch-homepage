...
::: info 音楽理論に詳しくない方へ
このページでは音楽理論の専門用語を使用しています。詳しくなくても心配不要です。MIDI Sketchは複雑なハーモニーを自動で処理します。これらの概念を理解すると、より良い選択ができるようになりますが、素晴らしい音楽を作るのに必須ではありません。
:::
...
::: info ボイスリーディングとは？
ボイスリーディングとは、個々の音が一つのコードから次のコードへどのように動くかということです。良いボイスリーディングは滑らかで繋がりのあるコード遷移を生み出します。悪いボイスリーディングはぎこちなくアマチュア的に聞こえます。MIDI Sketchは生成される全てのコード進行にプロフェッショナルなボイスリーディングを自動適用します。
:::
...
- **ルートレス**: ルート音を省略（ベースが弾く）。クリアで濁りのないサウンドを生む。プロのアレンジテクニック。
:::
...
| Expressive | 弱拍で9th、13th | カラフル、感情的、洗練 |
